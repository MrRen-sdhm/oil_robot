//! @file 			SerialPort.cpp
//! @author 		Geoffrey Hunter https://github.com/gbmhunter/CppLinuxSerial
...
namespace LinuxSerialPort {
	...
	{
		std::cout << "[INFO] Attempting to open COM port \"" << device_ << "\"." << std::endl;

		if(device_.empty()) {
//			THROW_EXCEPT("Attempted to open file when file path has not been assigned to.");
            printf("\033[1;31m[ERRO] Attempted to open file when file path has not been assigned to.\033[0m\n");
            return false;
		}
...
//		    THROW_EXCEPT("Could not open device " + device_ + ". Is the device name correct and do you have read/write permission?");
            printf("\033[1;31m[ERRO] Could not open device %s. Is the device name correct and do you have read/write permission?.\033[0m\n", device_.c_str());
            return false;
		}
    ...
    void SerialPort::Write(const unsigned char *data, int datalen) {
        ...
        int writeResult = write(fileDesc_, data, datalen);
    ...
    int SerialPort::Read(unsigned char * data)
	{
		...
		ssize_t n = read(fileDesc_, &data[0], readBufferSize_B_);
