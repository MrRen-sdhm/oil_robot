...
class OilPickup:
    ...
    def hand_control_client(self, command, pose=None):
        if pose is None:
            pose = -1
        else:
            pose = int(pose)
    ...
    def get_grasps_client(self):
        rospy.wait_for_service('/detect_grasps_server/detect_grasps', timeout=1)
        try:
            detectGrasps = rospy.ServiceProxy('/detect_grasps_server/detect_grasps', detect_grasps)
            resp = detectGrasps()
    ...
    def go_to_pose_goal_service(self):
      ...
      while not rospy.is_shutdown():
        ...
        for grasp in grasps:
          ...
          # rotate 0 or 180 to find a better traj
          better_traj, better_traj_len = self.find_better_traj(pose_goal)
          if better_traj_len > 0 and better_traj_len < 40:
            plan = group.execute(better_traj, wait=True)
    ...
    def cartesian_move(self, dir, scale):
      ...
      self.move_cartesian(waypoints, scale) 
...
if __name__ == "__main__":
  try:
    ...
    oil_pickup.group.set_named_target('pick_8')
    oil_pickup.group.go()
    ...
    for i in range(2):
      ...
      oil_pickup.cartesian_move(dir=-1, scale=0.3)
      # oil_pickup.group.set_named_target('middle_1')
      # oil_pickup.group.go()

      exit()

      oil_pickup.group.set_named_target('place_1')
      oil_pickup.group.go()
